...
export class VillageLedgerGame {
  ...
  private musicPanelBounds: { x: number; y: number; w: number; h: number } | null = null;
  private goldRecordAvatarPanelBounds: { x: number; y: number; w: number; h: number } | null = null;
  ...
  private settingsSubmenu: 'main' | 'sound' | 'howtoplay' | 'credits' = 'main';
  private showResetConfirm: boolean = false;
  private settingsPanelButtons: Array<{ x: number; y: number; w: number; h: number; action: string }> = [];
  private musicVolumeSliderArea: { x: number; y: number; w: number; h: number } | null = null;
  private sfxVolumeSliderArea: { x: number; y: number; w: number; h: number } | null = null;
  ...
  private playerManuallySelectedGenre: boolean = false;
  private shufflePlaylistActive: boolean = false;
  ...
  constructor(canvas: HTMLCanvasElement, onStateChange?: (state: GameState) => void) {
    ...
    const savedBadges = JSON.parse(localStorage.getItem('makingMoney_earnedBadges') || '[]');
  ...
  private processTouchStart(clientX: number, clientY: number): void {
    ...
    // Check gear/settings button FIRST so it works on ALL screens/overlays
    if (this.muteButtonArea) {
      const btn = this.muteButtonArea;
      if (x >= btn.x && x <= btn.x + btn.w && y >= btn.y && y <= btn.y + btn.h) {
        this.showSettingsPanel = !this.showSettingsPanel;
        this.settingsSubmenu = 'main';
    ...
    // Handle settings panel interactions (must be checked early, on all screens)
    if (this.showSettingsPanel) {
      if (this.settingsSubmenu === 'sound') {
        if (this.musicVolumeSliderArea) {
          const s = this.musicVolumeSliderArea;
          if (x >= s.x - 10 && x <= s.x + s.w + 10 && y >= s.y - 5 && y <= s.y + s.h + 5) {
            const val = Math.max(0, Math.min(1, (x - s.x) / s.w));
            soundManager.setMusicVolume(val);
            this.isDraggingMusicSlider = true;
            return;
          }
        }
        if (this.sfxVolumeSliderArea) {
          const s = this.sfxVolumeSliderArea;
          if (x >= s.x - 10 && x <= s.x + s.w + 10 && y >= s.y - 5 && y <= s.y + s.h + 5) {
            const val = Math.max(0, Math.min(1, (x - s.x) / s.w));
      ...
      for (const btn of this.settingsPanelButtons) {
        if (x >= btn.x && x <= btn.x + btn.w && y >= btn.y && y <= btn.y + btn.h) {
          soundManager.play('buttonClick');
          switch (btn.action) {
            case 'close':
            ...
            case 'sound':
              ...
              break;
            case 'howtoplay':
              this.settingsSubmenu = 'howtoplay';
              break;
            case 'credits':
              this.settingsSubmenu = 'credits';
              break;
            case 'fullscreen':
      ...
      const panelY = (this.logicalHeight - panelH) / 2;
    ...
    if (!this.state.showQuiz && !this.state.showQuizReview) {
      ...
      if (this.goldRecordPlaqueArea && this.discoSpriteUnlocked) {
      ...
      if (this.showGoldRecordAvatarPanel) {
        if (this.goldRecordAvatarPanelBounds) {
          const gp = this.goldRecordAvatarPanelBounds;
    ...
    if (this.state.showMusicCollection) {
      if (this.musicPanelBounds) {
        const mp = this.musicPanelBounds;
  ...
  private processTouchMove(clientX: number, clientY?: number): void {
    ...
    if (this.isDraggingMusicSlider && this.musicVolumeSliderArea && this.showSettingsPanel) {
      const s = this.musicVolumeSliderArea;
      const val = Math.max(0, Math.min(1, (x - s.x) / s.w));
      soundManager.setMusicVolume(val);
      return;
    }
    if (this.isDraggingSfxSlider && this.sfxVolumeSliderArea && this.showSettingsPanel) {
      const s = this.sfxVolumeSliderArea;
      const val = Math.max(0, Math.min(1, (x - s.x) / s.w));
      soundManager.setSfxVolume(val);
      return;
      ...
      this.queueDialogue([
        ...
        },
        {
          ...
          onComplete: () => {
            ...
            const completionCountInv = parseInt(localStorage.getItem('makingMoney_completionCount') || '0');
            if (!this.state.inventoryHintShown && completionCountInv === 0) {
  ...
  private update(dt: number): void {
    ...
    if (this.state.phase === 'complete_success' && this.player.x <= this.playerHomeX + 50 && 
        !this.state.showNightTransition && 
        !this.state.playerFading && !this.state.playerEnteredHut &&
    ...
    if (this.state.showCelebration) {
      ...
      if (this.partySongEndTime > 0 && Date.now() >= this.partySongEndTime && this.state.showCelebration) {
        if (this.shufflePlaylistActive) {
          this.playNextShuffledSong();
        } else if (this.playerManuallySelectedGenre && this.state.selectedGenre && this.state.selectedGenre !== 'background' && this.state.selectedGenre !== 'none') {
          const genreUrl = this.GENRE_AUDIO_MAP[this.state.selectedGenre];
          if (genreUrl) {
            soundManager.loadAndPlayGenre(genreUrl);
            this.partySongEndTime = Date.now() + 240000;
            setTimeout(() => {
      ...
      const djHintPlayCount = parseInt(localStorage.getItem('makingMoney_completionCount') || '0');
      if (djHintPlayCount === 0 && this.state.celebrationTimer > 20 && !this.state.djQuizPassed && !this.state.currentDialogue && !this.djHintShown) {
  ...
  private readonly SKIP_TO_PARTY_ENABLED = true;
  ...
  private startDiscoParty(): void {
    ...
    soundManager.stopAllMusic();
    soundManager.stopDaytimeMusic();
    const hintPlayCount = parseInt(localStorage.getItem('makingMoney_completionCount') || '0');
    if (hintPlayCount >= 1) {
      ...
      if (!hit && proj.active) {
        ...
        if (dbDist < discoBallR + proj.radius) {
          ...
          if (!this.state.slingshotDiscoBallRecordAwarded) {
            ...
            this.discoDestroyerShownStartTime = Date.now();
            this.awardRandomRecord(3000);
          }
          if (this.state.slingshotScore >= this.SLINGSHOT_TARGET_SCORE && !this.state.slingshotScoreRecordAwarded) {
            this.state.slingshotScoreRecordAwarded = true;
            this.partyChampionStartTime = Date.now();
  ...
  private drawSlingshotGame(ctx: CanvasRenderingContext2D): void {
    ...
    if (this.state.slingshotLocked) {
      ...
      if (!this.state.slingshotScoreRecordAwarded) {
        ...
        if (this.partyChampionStartTime > 0) {
          ...
          if (champElapsed < 2.5) {
            const champAlpha = champElapsed < 2 ? 1 : 1 - (champElapsed - 2) / 0.5;
            ctx.save();
            ctx.globalAlpha = Math.max(0, champAlpha);
            ctx.textAlign = 'center';
    ...
    if (t < 4 && platformScreenX > -100 && platformScreenX < w + 100) {
      ...
      if (this.slingshotPlatformActive) {
        ctx.fillText('Pop balloons to earn a record!', platformScreenX, groundY - platformH - 80);
      } else {
  ...
  private drawCelebrationAnimation(ctx: CanvasRenderingContext2D): void {
    ...
    if (this.state.partyEnded && !this.state.showCelebration) {
      ...
      const worldXPositions = [
        this.villageCenterX - 550,
        this.villageCenterX + 550,
        this.villageCenterX - 350,
        this.villageCenterX + 350
      ];
      for (let i = 0; i < 4; i++) {
        const isRight = (i % 2 === 1);
    ...
    const djFacing = this.villageElder.facingDirection || 1;
  ...
  private awardBadge(name: string, description: string, onDismiss?: () => void): void {
    const savedBadges = JSON.parse(localStorage.getItem('makingMoney_earnedBadges') || '[]');
    if (this.state.badges.includes(name) || savedBadges.includes(name)) {
  ...
  private dismissBadgePopup(): void {
    ...
    if (this.state.pendingBadge) {
      ...
      localStorage.setItem('makingMoney_earnedBadges', JSON.stringify(this.state.badges));
    }
  ...
  private drawBadgeTrayIcon(ctx: CanvasRenderingContext2D): void {
    if (this.state.showDJQuiz || this.state.showQuiz || this.state.showFail || this.state.showBrawl || this.state.showQuizReview || this.state.showSuccess) return;
  ...
  private silentCheckMusicScholar(): void {
    const unlocked = JSON.parse(localStorage.getItem('makingMoney_unlockedGenres') || '[]');
    const allGenres = Object.keys(this.GENRE_AUDIO_MAP);
  ...
  private drawGoldRecordAward(ctx: CanvasRenderingContext2D): void {
    ...
    ctx.fillStyle = `rgba(0, 0, 0, ${0.85 * fadeIn})`;
    ctx.fillRect(0, 0, w, h);

    if (fadeIn < 1) return;

    const contentAlpha = Math.min(1, (elapsed - 0.5) / 0.5);
    ...
    ctx.fillText("You've unlocked the Disco Avatar!", w / 2, h * 0.80);
    ctx.fillText('Toggle it from the Gold Record icon', w / 2, h * 0.84);
    ctx.fillText('on the HUD.', w / 2, h * 0.88);
  ...
  private drawGoldRecordPlaqueIcon(ctx: CanvasRenderingContext2D): void {
    if (!this.discoSpriteUnlocked) return;
    ...
    const iconY = mcIcon.y + (mcIcon.h - iconH) / 2 - 5;
  ...
  private drawGoldRecordHintArrow(ctx: CanvasRenderingContext2D): void {
    ...
    ctx.fillText('Tap for info!', arrowX, arrowY + 32 + bounce);
    
    ctx.restore();
  }

  private drawGoldRecordAvatarPanel(ctx: CanvasRenderingContext2D): void {
    if (!this.showGoldRecordAvatarPanel || !this.goldRecordPlaqueArea) return;

    const iconArea = this.goldRecordPlaqueArea;
    const panelW = 180;
    const panelH = 60;
    const panelX = iconArea.x + iconArea.w / 2 - panelW / 2;
    const panelY = iconArea.y + iconArea.h + 8;
    const clampedX = Math.max(8, Math.min(this.logicalWidth - panelW - 8, panelX));

    ctx.fillStyle = 'rgba(61, 40, 23, 0.95)';
    ctx.beginPath();
    ctx.roundRect(clampedX, panelY, panelW, panelH, 8);
    ctx.fill();
    ctx.strokeStyle = '#8B6914';
    ctx.lineWidth = 1.5;
    ctx.stroke();
    ...
    ctx.fillStyle = '#FFD700';
    ctx.textAlign = 'center';
    ctx.fillText('GOLD RECORDS', clampedX + panelW / 2, panelY + 20);

    ctx.font = `8px ${this.uiFont}`;
    ctx.fillStyle = '#C4A77D';
    ctx.fillText('All songs collected!', clampedX + panelW / 2, panelY + 38);

    this.goldRecordAvatarPanelBounds = { x: clampedX, y: panelY, w: panelW, h: panelH };
    ctx.textAlign = 'left';
  }
  ...
  private drawRecordRewardPopup(ctx: CanvasRenderingContext2D): void {
    ...
    const popupY = (playAreaH - popupH) / 2 - 15;
    ...
    for (let i = 0; i < sparkleCount; i++) {
      ...
      const sy = popupY + popupH / 2 + Math.sin(angle) * dist;
      const sparkleSize = 2 + Math.sin(elapsed * 0.008 + i * 2) * 1.5;
      ctx.fillStyle = `rgba(255, 215, 0, ${0.5 + Math.sin(elapsed * 0.006 + i) * 0.3})`;
      ctx.beginPath();
    ...
    ctx.beginPath();
    ctx.arc(cx - recordR * 0.3, cy - recordR * 0.3, recordR * 0.4, 0, Math.PI * 2);
    ctx.fill();

    ctx.font = `bold 16px ${this.uiFont}`;
    ctx.fillStyle = '#3D2914';
    ctx.textAlign = 'center';
    ctx.fillText('Record Collected!', cx, popupY + 155);
  ...
  private drawMusicCollectionPanel(ctx: CanvasRenderingContext2D): void {
    ...
    ctx.beginPath();
    ctx.roundRect(bgBtnX, shuffleBtnY, bgBtnW, bgBtnH, 6);
    ctx.fill();
    ctx.strokeStyle = isShuffleActive ? '#9B59B6' : '#666';
    ctx.lineWidth = isShuffleActive ? 2 : 1;
    ctx.stroke();
    ctx.font = `10px ${this.retroFont}`;
    ctx.fillStyle = isShuffleActive ? '#D7BDE2' : '#F5DEB3';
    ctx.textAlign = 'center';
    ctx.fillText('Shuffle All', bgBtnX + bgBtnW / 2, shuffleBtnY + bgBtnH / 2 + 4);
    if (isShuffleActive) {
      ctx.font = `bold 8px ${this.uiFont}`;
      ctx.fillStyle = '#9B59B6';
      ctx.fillText('SHUFFLING', bgBtnX + bgBtnW / 2, shuffleBtnY - 2);
    }
    this.musicCollectionShuffleBtn = { x: bgBtnX, y: shuffleBtnY, w: bgBtnW, h: bgBtnH };

    let discoSectionEndY = shuffleBtnY + bgBtnH;
    this.discoAvatarToggleBtn = null;
    if (!this.discoSpriteUnlocked) {
      const hintY = shuffleBtnY + bgBtnH + 15;
      ctx.save();
      const silW = 40;
      const silH = 50;
    ...
    if (this.discoSpriteUnlocked) {
      const dividerY = shuffleBtnY + bgBtnH + 12;
      ctx.strokeStyle = '#555';
      ctx.lineWidth = 1;
      ctx.beginPath();
  ...
  private handleMusicCollectionTouch(x: number, y: number): void {
    ...
    if (this.musicCollectionBgMusicBtn) {
      ...
      if (x >= btn.x && x <= btn.x + btn.w && y >= btn.y && y <= btn.y + btn.h) {
        soundManager.play('buttonClick');
        this.playerManuallySelectedGenre = false;
        this.shufflePlaylistActive = false;
        soundManager.stopAllMusic();
    ...
    if (this.musicCollectionShuffleBtn) {
      const btn = this.musicCollectionShuffleBtn;
      if (x >= btn.x && x <= btn.x + btn.w && y >= btn.y && y <= btn.y + btn.h) {
        soundManager.play('buttonClick');
        if (this.shufflePlaylistActive) {
          this.shufflePlaylistActive = false;
    ...
    for (const btn of this.musicCollectionBtns) {
      if (x >= btn.x && x <= btn.x + btn.w && y >= btn.y && y <= btn.y + btn.h) {
        ...
        this.playerManuallySelectedGenre = true;
        this.shufflePlaylistActive = false;
        soundManager.stopAllMusic();
  ...
  private startShufflePlaylist(): void {
    ...
    this.shuffleQueueIndex = 0;
    this.shufflePlaylistActive = true;
    this.playerManuallySelectedGenre = false;
    
    const genre = this.shuffleQueue[0];
    this.state.selectedGenre = genre;
    soundManager.stopAllMusic();
    const genreUrl = this.GENRE_AUDIO_MAP[genre];
    if (genreUrl) {
      soundManager.loadAndPlayGenre(genreUrl);
      this.partySongEndTime = Date.now() + 240000;
      setTimeout(() => {
  ...
  private playNextShuffledSong(): void {
    this.shuffleQueueIndex++;
    if (this.shuffleQueueIndex >= this.shuffleQueue.length) {
      for (let i = this.shuffleQueue.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [this.shuffleQueue[i], this.shuffleQueue[j]] = [this.shuffleQueue[j], this.shuffleQueue[i]];
      }
    ...
    const genre = this.shuffleQueue[this.shuffleQueueIndex];
    this.state.selectedGenre = genre;
    const genreUrl = this.GENRE_AUDIO_MAP[genre];
    if (genreUrl) {
      soundManager.loadAndPlayGenre(genreUrl);
...
private drawCharacter(ctx: CanvasRenderingContext2D, char: Character): void {
  ...
  if (processedSprite) {
    ...
    if (isDJing) {
      ...
      ctx.translate(Math.round(screenX), Math.round(y + spriteH - headBob - 10));
      if (facing < 0) {
        ctx.scale(-1, 1);
      }
  ...
  private drawGearButton(ctx: CanvasRenderingContext2D): void {
    const size = 36;
    const gearX = this.logicalWidth - size - 12;
    const gearY = 16;
    this.muteButtonArea = { x: gearX, y: gearY, w: size, h: size };

    ctx.fillStyle = this.showSettingsPanel ? 'rgba(139, 105, 20, 0.9)' : 'rgba(93, 72, 55, 0.85)';
    ctx.beginPath();
    ctx.roundRect(gearX, gearY, size, size, 8);
    ctx.fill();
    ctx.strokeStyle = '#8B6914';
    ctx.lineWidth = 2;
    ctx.stroke();

    const cx = gearX + size / 2;
    const cy = gearY + size / 2;
    const outerR = 11;
    const innerR = 7;
    const teeth = 8;
  ...
  private drawSettingsPanel(ctx: CanvasRenderingContext2D): void {
    ...
    if (this.showResetConfirm) {
      this.drawResetConfirmDialog(ctx, panelX, panelY, panelW, panelH);
    } else if (this.settingsSubmenu === 'sound') {
      this.drawSoundSubmenu(ctx, panelX, panelY, panelW, panelH);
    } else if (this.settingsSubmenu === 'howtoplay') {
      this.drawHowToPlay(ctx, panelX, panelY, panelW, panelH);
    } else if (this.settingsSubmenu === 'credits') {
      this.drawCredits(ctx, panelX, panelY, panelW, panelH);
    } else {
      this.drawMainMenu(ctx, panelX, panelY, panelW, panelH);
    }
  }

  private drawMainMenu(ctx: CanvasRenderingContext2D, px: number, py: number, pw: number, ph: number): void {
    ctx.font = `bold 14px ${this.uiFont}`;
    ctx.fillStyle = '#FFD700';
    ctx.textAlign = 'center';
    ...
    const menuItems = [
      { label: 'Sound Settings', action: 'sound' },
      ...
      { label: 'Reset Records', action: 'reset' },
    ];
    ...
    for (const item of menuItems) {
      ...
      ctx.fill();
      ctx.strokeStyle = item.action === 'reset' ? '#B43C3C' : '#8B6914';
      ctx.lineWidth = 1;
      ctx.stroke();

      ctx.font = `11px ${this.uiFont}`;
      ctx.fillStyle = item.action === 'reset' ? '#FF9999' : '#D4A574';
      ctx.textAlign = 'center';
      ctx.fillText(item.label, btnX + btnW / 2, btnY + btnH / 2 + 4);

      if (item.action === 'sound') {
        ctx.textAlign = 'right';
        ctx.fillText('>', btnX + btnW - 8, btnY + btnH / 2 + 4);
      }

      this.settingsPanelButtons.push({ x: btnX, y: btnY, w: btnW, h: btnH, action: item.action });
      btnY += btnH + gap;
    }
  ...
  private drawSoundSubmenu(ctx: CanvasRenderingContext2D, px: number, py: number, pw: number, ph: number): void {
    ctx.font = `bold 14px ${this.uiFont}`;
    ctx.fillStyle = '#D4A574';
    ctx.textAlign = 'left';
    ctx.fillText('< Back', px + 15, py + 28);
    this.settingsPanelButtons.push({ x: px + 8, y: py + 10, w: 80, h: 28, action: 'back' });

    ctx.fillStyle = '#FFD700';
    ctx.textAlign = 'center';
    ctx.fillText('SOUND', px + pw / 2, py + 28);
    ...
    ctx.fillStyle = '#A89070';
    ctx.textAlign = 'left';
    ctx.fillText('Music Volume', sliderX, curY);
    curY += 14;
    this.drawVolumeSlider(ctx, sliderX, curY, sliderW, 16, soundManager.getMusicVolume(), 'music');
    this.musicVolumeSliderArea = { x: sliderX, y: curY, w: sliderW, h: 16 };
    curY += 28;

    this.drawMuteToggle(ctx, sliderX, curY, sliderW, 'Music', soundManager.isMusicMuted(), 'toggleMusicMute');
    curY += 36;

    ctx.font = `10px ${this.uiFont}`;
    ctx.fillStyle = '#A89070';
    ctx.textAlign = 'left';
    ctx.fillText('SFX Volume', sliderX, curY);
    curY += 14;
    this.drawVolumeSlider(ctx, sliderX, curY, sliderW, 16, soundManager.getSfxVolume(), 'sfx');
    this.sfxVolumeSliderArea = { x: sliderX, y: curY, w: sliderW, h: 16 };
    curY += 28;

    this.drawMuteToggle(ctx, sliderX, curY, sliderW, 'SFX', soundManager.isSfxMuted(), 'toggleSfxMute');

    ctx.textAlign = 'left';
  }

  private drawVolumeSlider(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, value: number, type: string): void {
    ...
    ctx.fillStyle = '#8B6914';
    ctx.beginPath();
    ctx.roundRect(x, y + h / 2 - 3, fillW, 6, 3);
    ctx.fill();
    ...
    ctx.fillStyle = '#D4A574';
    ctx.textAlign = 'right';
    ctx.fillText(`${Math.round(value * 100)}%`, x + w + 1, y + h / 2 + 3);
    ctx.textAlign = 'left';
  }

  private drawMuteToggle(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, label: string, muted: boolean, action: string): void {
    const toggleW = 40;
    const toggleH = 20;
    ...
    ctx.fillStyle = '#A89070';
    ctx.textAlign = 'left';
    ctx.fillText(`Mute ${label}`, x, y + toggleH / 2 + 3);

    ctx.fillStyle = muted ? 'rgba(180, 60, 60, 0.6)' : 'rgba(34, 120, 60, 0.6)';
    ctx.beginPath();
    ctx.roundRect(toggleX, toggleY, toggleW, toggleH, toggleH / 2);
    ctx.fill();
    ctx.strokeStyle = muted ? '#B43C3C' : '#2D8B47';
    ctx.lineWidth = 1;
    ctx.stroke();

    const thumbR = toggleH / 2 - 2;
    const thumbCX = muted ? toggleX + toggleH / 2 : toggleX + toggleW - toggleH / 2;
    ctx.fillStyle = '#FFFFFF';
    ctx.beginPath();
    ctx.arc(thumbCX, toggleY + toggleH / 2, thumbR, 0, Math.PI * 2);
    ctx.fill();

    this.settingsPanelButtons.push({ x: toggleX, y: toggleY, w: toggleW, h: toggleH, action });
  }

  private drawResetConfirmDialog(ctx: CanvasRenderingContext2D, px: number, py: number, pw: number, ph: number): void {
    const cx = px + pw / 2;

    ctx.font = `bold 12px ${this.uiFont}`;
    ctx.fillStyle = '#FF6666';
    ctx.textAlign = 'center';
    ctx.fillText('RESET ALL RECORDS?', cx, py + 40);

    ctx.font = `9px ${this.uiFont}`;
    ctx.fillStyle = '#D4A574';
    ctx.fillText('This will erase all unlocked genres,', cx, py + 65);
    ctx.fillText('badges, your name, and play history.', cx, py + 80);
    ctx.fillText('This cannot be undone!', cx, py + 100);

    const btnW = 100;
    const btnH = 32;
    const confirmX = cx - btnW - 10;
    const confirmY = py + 120;
    ...
    ctx.fillText('Yes, Reset', confirmX + btnW / 2, confirmY + btnH / 2 + 4);
    this.settingsPanelButtons.push({ x: confirmX, y: confirmY, w: btnW, h: btnH, action: 'confirmReset' });

    const cancelX = cx + 10;
    ctx.fillStyle = 'rgba(139, 105, 20, 0.3)';
    ctx.beginPath();
    ctx.roundRect(cancelX, confirmY, btnW, btnH, 6);
    ctx.fill();
    ctx.strokeStyle = '#8B6914';
    ctx.lineWidth = 1;
    ctx.stroke();
    ctx.fillStyle = '#D4A574';
    ctx.fillText('Cancel', cancelX + btnW / 2, confirmY + btnH / 2 + 4);
    this.settingsPanelButtons.push({ x: cancelX, y: confirmY, w: btnW, h: btnH, action: 'cancelReset' });

    ctx.textAlign = 'left';
  }

  private drawHowToPlay(ctx: CanvasRenderingContext2D, px: number, py: number, pw: number, ph: number): void {
    ctx.font = `bold 14px ${this.uiFont}`;
    ctx.fillStyle = '#D4A574';
    ctx.textAlign = 'left';
    ctx.fillText('< Back', px + 15, py + 28);
    ...
    ctx.fillStyle = '#FFD700';
    ctx.textAlign = 'center';
    ctx.fillText('HOW TO PLAY', px + pw / 2, py + 28);
  ...
  private drawCredits(ctx: CanvasRenderingContext2D, px: number, py: number, pw: number, ph: number): void {
    ctx.font = `bold 14px ${this.uiFont}`;
    ctx.fillStyle = '#D4A574';
    ctx.textAlign = 'left';
    ctx.fillText('< Back', px + 15, py + 28);
    ...
    ctx.fillStyle = '#FFD700';
    ctx.textAlign = 'center';
    ctx.fillText('CREDITS', px + pw / 2, py + 28);

    ctx.font = `11px ${this.uiFont}`;
    ctx.fillStyle = '#D4A574';
    ctx.fillText('Making Money', px + pw / 2, py + 65);
    ctx.fillText('The Barter System', px + pw / 2, py + 85);

    ctx.font = `9px ${this.uiFont}`;
    ctx.fillStyle = '#A89070';
    ctx.fillText('Created by', px + pw / 2, py + 115);

    ctx.font = `bold 12px ${this.uiFont}`;
    ctx.fillStyle = '#FFD700';
    ctx.fillText('Shalolly Inc.', px + pw / 2, py + 135);
  ...
  private drawDJModeButton(ctx: CanvasRenderingContext2D): void {
    ...
    const boothWorldX = this.djBoothWorldX || this.villageElder.x;
    const boothScreenX = boothWorldX - this.cameraX;
    const groundY = this.logicalHeight - this.groundHeight - this.dialogueBoxHeight;
    const btnW = 50;
    const btnH = 28;
    const btnX = boothScreenX - btnW / 2;
    const btnY = groundY + 25;
  ...
  private drawPartyHint(ctx: CanvasRenderingContext2D): void {
    ...
    const hintMaxWidth = w - textStartX - 20;
    const hintWords = this.state.partyHintText.split(' ');
    let hintLine = '';
    let hintLineY = boxY + 62;
    for (const word of hintWords) {
      const testLine = hintLine + word + ' ';
      if (ctx.measureText(testLine).width > hintMaxWidth && hintLine !== '') {
        ctx.fillText(hintLine.trim(), textStartX, hintLineY);
        hintLine = word + ' ';
        hintLineY += 20;
      } else {
        hintLine = testLine;
  ...
  private drawCheckpointQuiz(ctx: CanvasRenderingContext2D): void {
    ...
    const estimatedContentH = 65 + (numOptions * 56) + 80;
    const cardH = Math.max(340, estimatedContentH);
    const cardX = (w - cardW) / 2;
    const cardY = (h - cardH) / 2;
    ...
    if (this.state.checkpointQuizAnswered) {
      ...
      const btnH = 34;
      const btnX = (w - btnW) / 2;
      const btnY = Math.max(cardY + cardH - 48, explLineY + 18);
  ...
  private drawDJQuiz(ctx: CanvasRenderingContext2D): void {
    ...
    ctx.font = `11px ${this.retroFont}`;
    const preWrapLines = this.wrapTextLines(ctx, q.question, cardW - 50);
    const questionTextHeight = preWrapLines.length * 18;
    let hintHeight = 0;
    if (this.djQuizShowHint && !this.djQuizShowResult) {
      ctx.font = `italic 10px ${this.uiFont}`;
      const hintLines = this.wrapTextLines(ctx, "Hint: " + q.hint, cardW - 60);
      hintHeight = hintLines.length * 16 + 10;
    }
    const optionCount = q.options.length;
    const optionHeight = optionCount * 48;
    const baseCardH = this.djQuizShowResult ? 340 : (60 + questionTextHeight + hintHeight + 12 + optionHeight + 70);
    const cardH = Math.max(this.djQuizShowResult ? 340 : 300, Math.min(baseCardH, h - 40));
    const cardX = (w - cardW) / 2;
    const cardY = (h - cardH) / 2;
  ...
  private drawQuizOverlay(ctx: CanvasRenderingContext2D): void {
    ...
    ctx.font = `11px ${this.retroFont}`;
    const preQuizLines = this.wrapTextLines(ctx, q.question, cardW - 60);
    const questionBlockH = 100 + preQuizLines.length * 20 + 30;
    const reflectionH = reflectionAnswer ? 80 : 0;
    const isMultiSelectQ = q.multiSelect === true;
    const btnSpacingCalc = isMultiSelectQ ? 48 : 58;
    const submitBtnH = isMultiSelectQ ? 55 : 0;
    const neededH = questionBlockH + reflectionH + optionCount * btnSpacingCalc + submitBtnH + 20;
    const cardH = Math.max(400, Math.min(neededH, h - 40));
    const cardX = (w - cardW) / 2;
    const cardY = (h - cardH) / 2;
  ...
  private drawSongChoice(ctx: CanvasRenderingContext2D): void {
    ...
    const allCollected = unlockedCount >= allGenres.length;
    const genres = allCollected 
      ? allGenres 
      : allGenres.filter(g => g !== this.state.selectedGenre && !unlocked.includes(g));

    ctx.font = `bold 18px ${this.uiFont}`;
    ctx.textAlign = 'center';
    ctx.fillStyle = '#FFD700';
    ctx.fillText(allCollected ? 'Pick Any Celebration Song!' : 'Choose Your Celebration Song!', w / 2, cardY + 35);
    
    ctx.font = `12px ${this.uiFont}`;
    ctx.fillStyle = '#C4A77D';
    ctx.fillText(allCollected ? 'All records collected! Pick any song!' : 'Pick a genre for your victory remix', w / 2, cardY + 55);
    const btnW = 110;
    const btnH = 40;
    const gapX = 15;
    const gapY = 8;
  ...
  private advanceEndGameSequence(): void {
    ...
    if (this.showDiscoAvatarCelebration) return;
    this.pendingSuccessAfterPopups = false;
