...
- **Audio System**: Web Audio API with `AudioContext` for dynamic soundscapes, including pitch-varied footsteps, layered brawl audio, and dynamic background music. Separate music and SFX gain nodes with independent volume sliders and mute toggles, persisted to localStorage. Audio preloading and `resumeContext()` for compatibility.
...
- **Disco Avatar**: Players who earn the Music Scholar badge (all 11 records collected) unlock a disco outfit character sprite (`player-disco.png`). Toggle available in Music Collection panel and via Gold Record Plaque HUD icon. Pure cosmetic swap — same dimensions, animations, hitbox. Choice persisted in localStorage (`makingMoney_useDiscoSprite`).
- **Gold Record Award**: When player collects all 11 genres during party, a special "Gold Record" full-screen award presentation displays with dual sparkle effects (circular ring + floating golden stars), large disco avatar, DJ horn sound after reward, and explanation of how to toggle the disco avatar. Awarded during party only — returning players get silent unlock on game start.
- **Gold Record Plaque HUD Icon**: Gold vinyl record icon appears next to music collection icon once Music Scholar is earned. Tap to open a small avatar toggle panel for quick normal/disco sprite switching.
- **Settings Menu**: Gear icon in top-right corner (replaces mute button) opens a settings panel with: Sound Settings submenu (music volume slider, SFX volume slider, music mute toggle, SFX mute toggle), How to Play instructions, Credits (Shalolly Inc.), Fullscreen toggle, Restart Game, and Reset Records (with confirmation warning popup).
...
- **Reset Records**: Available in Settings menu (gear icon), clears `makingMoney_unlockedGenres`, `makingMoney_completionCount`, `makingMoney_playerName`, `makingMoney_useDiscoSprite`, and `makingMoney_postReflection` from localStorage. Requires confirmation before clearing.
