...
- **Badge System**: 5 collectible badges (Double Coincidence of Wants, Debt, The Ledger, Debt Settled, Money Scholar) with persistent tray HUD (X/5 progress), tap-to-expand panel, golden glow animation on earn, enhanced sparkle popup (multi-ring particles with glow), and badge-centric success screen with "Your Journey" reflection comparison. All 5 badges are earnable on both the normal path and the smart path in a single playthrough.
...
- **Quiz System**: Two distinct quiz types: (1) DJ Quiz - 1-2 questions during party (DCW/Debt topics) with hint-on-wrong, retry, second-chance backup question, and storm failure on double-fail; gates song genre choice. (2) Final Quiz - 4 ledger-focused review questions after hut/storm sequence, non-gated, leads directly to success screen. Questions sourced from centralized `quizBank.ts` (10 questions across 3 categories: dcw, debt, ledger). Plus distributed "Quick Check!" checkpoint pop-ups after brawl and first ledger recording.
...
- **Song Choice Mechanic**: After passing DJ Quiz during party, player picks from 10 celebration genres (Classic Hip-Hop, Rock, K-Pop, Reggae, Yacht Rock, 80's Hairband, Trap, Pop, 80's Funk, Disco) displayed as a 2-column color-coded button grid. Each genre has its own dedicated remix audio file, lazy-loaded on selection to minimize memory usage. Mid-party song change allowed once via DJ Elder interaction. Record scratch placeholder sound on song transitions.
- **Music Collection HUD**: Vinyl record icon on HUD (visible after first genre unlock) showing X/10 progress. Tap to expand collection panel with all 10 genres (unlocked highlighted, locked grayed out), standard background music option, and NOW PLAYING indicator. Unlocked genres persist across sessions via localStorage (`makingMoney_unlockedGenres`). Players can switch background music to any unlocked genre during gameplay.
