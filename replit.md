...
- **Badge System**: 5 collectible badges (Double Coincidence of Wants, Debt, The Ledger, Debt Settled, Money Scholar) with persistent tray HUD (X/5 progress), tap-to-expand panel (row of 3 + centered row of 2), golden glow animation on earn, circular sparkle popup (no rectangular glow), and badge-centric success screen with "Your Journey" reflection comparison. All 5 badges are earnable on both the normal path and the smart path in a single playthrough.
...
- **Quiz System**: Two distinct quiz types: (1) DJ Quiz - 1-2 questions during party (DCW/Debt topics) with hint-on-wrong, retry, second-chance backup question, and storm failure on double-fail; gates song genre choice. (2) Final Quiz - 4 ledger-focused review questions + 1 random extra credit bonus question (centralized vs decentralized) after hut/storm sequence, non-gated, leads directly to success screen. Questions sourced from centralized `quizBank.ts` (15 questions across 4 categories: dcw, debt, ledger, extra_credit). Plus distributed "Quick Check!" checkpoint pop-ups after brawl and first ledger recording.
...
- **Music Collection HUD**: Vinyl record icon on HUD positioned next to badge tray icon (visible after first genre unlock) showing X/10 progress with circular golden glow on new unlock. Tap to expand collection panel with all 10 genres (unlocked highlighted, locked grayed out), standard background music option, and NOW PLAYING indicator. Unlocked genres persist across sessions via localStorage (`makingMoney_unlockedGenres`). Players can switch background music to any unlocked genre during gameplay. Temporary "Reset Records" button on loading screen for testing.
